        ...
        <div className="flex items-center">
          ...
          <button
            className={`p-2 rounded-full bg-gray-200 hover:bg-gray-300 ${
              !isScrollable || scrollPosition >= scrollRef.current?.scrollWidth - scrollRef.current?.clientWidth
                ? 'opacity-50 cursor-not-allowed'
                : ''
            }`}
            onClick={() => handleScroll('right')}
            disabled={!isScrollable || scrollPosition >= scrollRef.current?.scrollWidth - scrollRef.current?.clientWidth}
          >
            <FiChevronRight className="h-5 w-5 text-gray-600" />
          </button>
        </div>
      </div>
      <div ref={scrollRef} className="flex overflow-x-auto space-x-4 pb-4 scroll-container">
